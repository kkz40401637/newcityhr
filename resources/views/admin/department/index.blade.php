        ...
        <div class="layout-px-spacing data-out-put-div">
            <div class="page-header">
                <div class="page-title">
                    <a href="{{route('AddDepartmentList')}}" class="d-inline btn btn-sm shadow-lg theme-bg theme-color "> ADD NEW DEPARTMENT + </a>&nbsp;&nbsp;&nbsp;
                    <h3 class="d-inline float-right" >DEPARTMENT lIST</h3>
                </div>
            </div>
                            ...
                            <div class="table-responsive mb-4">
                                <table id="style-3" class="table style-3  table-hover">
                                    ...
                                    <tr>
                                        ...
                                        <th><div class="th-content">BUSINESS UNIT</div></th>
                                        <th><div class="th-content">Branch</div></th>
                                        <th><div class="th-content">Inial</div></th>
                                            ...
                                            <td>
                                                <div class="td-content text-dark">
                                                  <span class="badge text-white badge-pills bg-primary">
                                                    {{$department->BusinessUnitInfo->Name}}</span>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="td-content text-dark">
                                                  <span class="badge text-white badge-pills bg-secondary">
                                                    {{$department->StationInfo->StationName}}</span>
                                                </div>
                                            </td>
                                            <td><div class="td-content text-dark">{{$department->Initial}}</div></td>
                                            <td>
                                                <a class="btn btn-primary position-relative " href="{{route('EditDepartmentInfo',$department->Token)}}">
                                                    <i class="material-icons ">edit</i>
                                                </a>
                                            </td>
                                            <td>
                                                <button onclick="DepartmentBanded({{$department->id}})" class="departmentId-ban-btn{{$department->id}}  text-white btn bg-solid-red position-relative  ">
...
<script>
    ...
    function DepartmentBanded(id)
    {
        $('.departmentId-ban-btn'+id).prop('disabled', true);
        console.log(id);
        $.post("{{route('DepartmentStatusChange')}}",
            {
                _token: csrf_token,
                DepartmentId: id
            },
