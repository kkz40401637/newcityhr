...
<div class="header-container fixed-top">
    ...
    <header class="header navbar navbar-expand-sm text-black">
        ...
        ?>
        <h6 class=" ml-3 mt-sm-2 text-black">{{!empty($RoleDb)?$RoleDb->Name:''}}</h6>

        <ul class="navbar-item flex-row ml-auto">

            <li class="nav-item dropdown user-profile-dropdown  order-lg-0 order-1">
                <a href="javascript:void(0);" class="nav-link dropdown-toggle user" id="userProfileDropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <i class="material-icons text-black" >person</i>
                </a>
...
{{--                                <p>{{$UserRole [Auth::user()->role-1] }}</p>--}}
                    ...
                    <div class="dropdown-item">
                        <form method="POST" action="{{route('logout')}}">
                            ...
                            <button type="submit" class="LogoutBtn" ><span>Log Out</span></button>
                        </form>
                    </div>
                </div>
            </li>
